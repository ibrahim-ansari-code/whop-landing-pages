...
- **GitHub agent: repo has App access but no commits**: The cron runs one adjust job at a time. If it’s stuck in a long Claude call, you see "maximum number of running instances reached" and no "Pushed" logs. Set `CLAUDE_CTA_ALIGN_TIMEOUT_SECONDS=300` in the python-agent `.env` so each variant request times out after 5 min. Check logs for "Calling Claude to align …", "Pushing …", "Pushed …" or "Failed to update …".
