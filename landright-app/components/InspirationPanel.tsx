...
export type InspirationGate = { submitBlocked: boolean; scanning: boolean };

interface InspirationPanelProps {
  onInspirationChange: (data: InspirationData | null) => void;
  /** Submit is blocked until scan succeeds when a screenshot + backend exist; `scanning` drives button label. */
  onInspirationGateChange?: (gate: InspirationGate) => void;
}

export function InspirationPanel({ onInspirationChange, onInspirationGateChange }: InspirationPanelProps) {
  ...
  const [scanSummary, setScanSummary] = useState<{ sections: number; triggers: number; elements: number }>({
    sections: 0,
    triggers: 0,
    elements: 0,
  });
  ...
  useEffect(() => {
    if (!onInspirationGateChange) return;
    const backendOk = Boolean(GENERATE_API_BASE);
    const hasShot = Boolean(screenshotDataUrl);
    const submitBlocked = Boolean(hasShot && backendOk && !scanned);
    const scanning = Boolean(hasShot && backendOk && extracting && !scanned);
    onInspirationGateChange({ submitBlocked, scanning });
  }, [screenshotDataUrl, scanned, extracting, onInspirationGateChange]);

  useEffect(() => {
    if (!screenshotDataUrl || !GENERATE_API_BASE) return;
    ...
    (async () => {
      ...
      try {
        const payload = await compressImageIfNeeded(dataUrl);
  ...
  }, [screenshotDataUrl, retryNonce, onInspirationChange]);
  ...
  function handleRetryScan() {
    if (!screenshotDataUrl || !GENERATE_API_BASE || extracting) return;
    setError(null);
    setRetryNonce((n) => n + 1);
  }

  const backendMissing = Boolean(screenshotDataUrl && !GENERATE_API_BASE);

  return (
    <div className="mt-8">
      <h3 className="text-sm font-medium text-zinc-300">Inspiration (optional)</h3>
      <p className="mt-1 text-xs text-zinc-500">
        Upload or paste a screenshot of a landing page. We scan it automatically and extract design DNA for your variants.
      </p>
      <div
          ...
          <div className="flex flex-col gap-3">
            <div className="relative flex items-start gap-3">
              ...
              <div className="flex flex-1 flex-wrap gap-2">
                ...
                {extracting && (
                  <span className="self-center text-xs text-zinc-400">{COPY.HOME.INSPIRATION_SCANNING}</span>
                )}
                {error && GENERATE_API_BASE && !extracting && (
                  <button
                    type="button"
                    onClick={handleRetryScan}
                    className="rounded-lg bg-zinc-700 px-4 py-2 text-sm font-medium text-zinc-100 hover:bg-zinc-600 transition"
                  >
                    {COPY.HOME.RETRY_INSPIRATION}
                  </button>
                )}
                {extracting && (
                  <button
      ...
      {backendMissing && (
        <p className="mt-2 text-xs text-amber-400">
          Backend URL is not configured; inspiration scan is skipped. You can still generate without it.
        </p>
      )}
      ...
      {scanned && (
        <div className="mt-4 rounded-xl border border-orange-500/30 bg-gradient-to-br from-zinc-900 to-zinc-950 p-4 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">
                {scanSummary.sections} sections &middot; {scanSummary.elements} elements &middot; {scanSummary.triggers}{" "}
                persuasion triggers
              </p>
            </div>
          </div>
          ...
          {palette.length > 0 && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {palette.map((c, i) => (
                <span
                  key={i}
                  className="inline-block h-6 w-6 rounded-md border border-zinc-600 shadow-sm"
          ...
          <p className="text-[10px] text-zinc-600 leading-tight">
            Theme overrides, design tokens, and persuasion triggers have been auto-applied. Your 4 variants will synthesize
            these patterns.
          </p>
        </div>
      )}
